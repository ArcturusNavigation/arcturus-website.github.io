...
#### Follow the Path (baseline, addition 1, addition 2 —> core)
...
- occasionally manage this without crashing into anything—suspect this is not a dynamic obstacle avoidance thing though

#### Speed (baseline + addition —> core)
...
- determined: pure GPS ≥ SLAM w/ low GPS uncertainty > LIO (for now)
- occasionally returns through gates — not always correct gate (but suspect this is quirk of testing with all the follow the path buoys still in the water)

#### Docking + Mechanisms (baseline)

**Tested: detection of shore mounted banners w/ boat in water —>**

- missing:
- failed to detect banners—not sure cause right now, but could be low light + backlit
...
| [delivery] baseline: mechanism navigation | 2 | F (no detect) | — |
...
#### SLAM (err @high velocities —> latency w/ odometry messages)

**problem: SLAM is supposed to rectify GPS drift but currently performs worse than raw GPS at high velocities**

- **Mismatched SLAM + Odometry Messages:** Latency in detections means odometry messages from queue don't "match" pose used when generating map?
    - seems like there's no  great solution to this
...
#### GPS (Localization —> significant drift?)
