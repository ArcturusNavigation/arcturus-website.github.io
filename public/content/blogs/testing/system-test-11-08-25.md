...
- Showed clear "intent" to follow path
...
- consistently navigated to buoy (when auto'd w/ buoy in view), generally circled it, consistently returned to home
- Errors attributed to problems with the retry logic —> failed to navigate to probe point + failed to generate path to first (& sometimes second) base point (results in incorrect turning behavior)
...
**BMS**
- Both BMSes failed during the test —> outputed < 1/2 the voltage of the batteries
...
**Speed**
- Fix retry logic —> check if this fixes selection of base points in sim. If not, fix base point selection
